...
class ConvEncoder(nn.Module):
    def __init__(self, out_dim=None):
        ...
        self.conv3 = torch.nn.Conv2d(64, 64, 4, 2, 1, bias=False)
        # self.conv4 = torch.nn.Conv2d(128, 1, 1, 1, 0) # 54*44

        self.LReLU = torch.nn.LeakyReLU(0.2, inplace=True)
        self.convm = torch.nn.Conv2d(3, 3, 4, 2, 1)
        self.convv = torch.nn.Conv2d(3, 3, 4, 2, 1)
        ...
        # self.fc1 = torch.nn.Linear(6*6*3, 512)
    ...
    def encode_simple(self, x):
        ...
        #print(hv.size())
        #hv = hv.view(-1, 8 * 8)
        mu, var = self.mean_layer(x), self.var_layer(x)
        var = F.softplus(var) + 1e-8
        #print(mu.size())
        #print(var.size())
        # print(x.shape)
