...
def get_dataset(name, root, image_size, norm=True, flip=False, **kwargs):
    ...
    if name == "celeba":
        ...
        dataset = FFHQDataset(root, **kwargs)
    elif name == "smile":
        dataset = SmileDataset(root)
    elif name == "cifar10":
...
def structural_condition_prior(scale, label, dim, A):
    # CHANGE TO WHEN WE ARE INTERVENING ON 3RD OR 4TH CONCEPT TO HARDCODE IT
...
def log_bernoulli_with_logits_bce(x, logits):
    """
    Computes the log probability of a Bernoulli given its logits
