...
from conv_encoder_decoder_shadow import ConvEncoder, ConvDec
...
def compute_importance_gbt(x_train, y_train, x_test, y_test):
    """Compute importance based on Gradient Boosting Trees (GBT)."""
    num_factors = y_train.shape[0]  # Number of causal factors (e.g., 3)
    num_codes = x_train.shape[0]  # Number of latent variables (Z)
    ...
    for i in range(num_factors):
        model = ensemble.GradientBoostingRegressor()  # Use GBT
        model.fit(x_train.T, y_train[i, :])  # Train on latent representations

        # Store feature importance
        importance_matrix[:, i] = np.abs(model.feature_importances_)

        # Compute accuracy/error for informativeness
...
def _compute_dci(mus_train, ys_train, mus_test, ys_test):
    """Computes DCI scores based on training/testing latent representations."""
    scores = {}

    # Compute feature importances and informativeness
    importance_matrix, train_err, test_err = compute_importance_gbt(
        mus_train, ys_train, mus_test, ys_test)

    # Ensure matrix dimensions match expectations
    assert importance_matrix.shape[0] == mus_train.shape[0]
    assert importance_matrix.shape[1] == ys_train.shape[0]

    # Store informativeness
...
class CausalVAE(pl.LightningModule):
    def __init__(self, model_name='cvae', z_dim=384, z1_dim=3, z2_dim=128, inference=False, alpha=0.1, beta=1, lambda_v=1e-5, lr=1e-4, use_causal_prior=True, warmup=100, max_iters=100, **kwargs):
        super().__init__()
        ...
        self.z2_dim = z2_dim
        self.channel = 3
        self.scale = np.array([[1.0, 0.7], [4, 4], [3.3, 3.3]])
        ...
        self.mask_u = mask.MaskLayer(self.z1_dim, z1_dim=3)
    ...
    def save_latent_representations(self, dataloader, epoch):
        """Saves latent representations for t-SNE visualization."""
        if epoch != 99:  # Only save at epoch 99
            return

        print(f"Saving latent representations at epoch {epoch}...")
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                z_sample, _, z_m, z_v = self.encode(X)

                # Sample from Gaussian distribution z ~ N(z_m, z_v)
                z = ut.conditional_sample_gaussian(z_m, z_v * self.lambda_v)

                # Convert to NumPy
                all_z_sample.append(z_sample.cpu().numpy())
                all_z.append(z.cpu().numpy())
                all_labels.append(labels.cpu().numpy())

        # Stack all batches
        all_z_sample = np.concatenate(all_z_sample, axis=0)
        all_z = np.concatenate(all_z, axis=0)
        all_labels = np.concatenate(all_labels, axis=0)

        # Save to file
        save_dir = "/home/2527823Y/DiffuseVAE-main/main/datasets/causal_result/npy"
        os.makedirs(save_dir, exist_ok=True)
        np.save(os.path.join(save_dir, "z_sample_epoch_99.npy"), all_z_sample)
        np.save(os.path.join(save_dir, "z_epoch_99.npy"), all_z)
        np.save(os.path.join(save_dir, "labels_epoch_99.npy"), all_labels)

        print(f"Latent representations saved in {save_dir}")
    ...
    def _get_loss(self, x, label, mode="train"):
        ...
        # RECONSTRUCTION LOSS
        rec_mse = ut.log_bernoulli_with_logits(x, x_hat.reshape(x.size()))
        rec_bce = ut.log_bernoulli_with_logits_bce(x, x_hat.reshape(x.size()))
        rec = 0.7 * rec_bce + 0.3 * rec_mse
        ...
        kl = torch.mean(kl)
        # dag_para = self.dag.A
        # h_a = _h_A(dag_para, dag_para.size()[0])
        neg_elbo = rec + 0.1 * kl # + 3 * h_a + 0.5 * h_a * h_a
    ...
    # def test_step(self, batch, batch_idx, mask=0, value=0.6):
    #
    #     X, label = batch
    #     print('label', label)
    #     #print('X', X)
    ...
    def test_step(self, batch, batch_idx, mask=1, value=7):
        """Performs a test step and saves each image separately."""
        X, label = batch
        # print(label)

        # Forward pass to get reconstructed/intervened images
        x_hat, z_sample, z_masked, z_m, z_v, label = self.forward(X, label, mask=mask, value=value)
        x_hat = x_hat.reshape(X.size())

        # Create directories to store images
        real_dir = os.path.join(self.logger.log_dir, "Real_Inference")
        recon_dir = os.path.join(self.logger.log_dir, "Reconstructions_Inference")
        interv_dir = os.path.join(self.logger.log_dir, "Interventions_Inference")
        ...
        for i in range(X.shape[0]):  # Loop through batch
            vutils.save_image(X[i], os.path.join(real_dir,
                                                 f"real_{self.logger.name}_Epoch_{self.current_epoch}_batch_{batch_idx}_img_{i}.png"),
                              normalize=True)
            vutils.save_image(x_hat[i], os.path.join(recon_dir,
                                                     f"recon_{self.logger.name}_Epoch_{self.current_epoch}_batch_{batch_idx}_img_{i}.png"),
                              normalize=True)

            if mask is not None:  # If intervention applied
                vutils.save_image(x_hat[i], os.path.join(interv_dir,
                                                         f"intervened_{self.logger.name}_Epoch_{self.current_epoch}_batch_{batch_idx}_img_{i}.png"),
                                  normalize=True)

        print(f"Saved {X.shape[0]} images from batch {batch_idx}.")
